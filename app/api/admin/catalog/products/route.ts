...
export async function POST(req: Request) {
  ...
  const attempt = await supabaseServer.from("products").insert([productData as any]).select("*").single()
  if (attempt.error) {
    const msg = attempt.error.message || ""
    if (/variant_dimensions/i.test(msg)) {
      const { variant_dimensions, ...legacy } = productData as any
      const attempt2 = await supabaseServer.from("products").insert([legacy]).select("*").single()
      if (attempt2.error) return NextResponse.json({ error: attempt2.error.message }, { status: 500 })
      const created2 = attempt2.data
      if (created2?.id && productData.image) {
        try {
          await syncPrimaryImage(created2.id, productData.image)
        } catch (e: any) {
          return NextResponse.json({ error: e?.message || "Failed to sync primary image", product: created2 }, { status: 500 })
        }
      }
      return NextResponse.json({ product: created2 })
    }
    return NextResponse.json({ error: attempt.error.message }, { status: 500 })
  }
  const created = attempt.data

  if (created?.id && productData.image) {
    try {
      await syncPrimaryImage(created.id, productData.image)
    } catch (e: any) {
      return NextResponse.json({ error: e?.message || "Failed to sync primary image", product: created }, { status: 500 })
    }
  }
